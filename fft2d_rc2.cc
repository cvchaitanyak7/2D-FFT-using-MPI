...
Complex **Create2dArray(int rows, int cols) 
{
...
{
	Complex** h=Create2dArray(nrows,ncols);
...
{
	int N=nrows;
	Complex** W=Create2dArray(nrows,ncols);
	for(int i=0; i<nrows; i++)
	{
		for(int j=0; j<ncols; j++)
		{
...
{
	int N=nrows;
	Complex** Wconj=Create2dArray(nrows,ncols);
	for(int i=0; i<nrows; i++)
	{
		for(int j=0; j<ncols; j++)
		{
...
void sendToAll(Complex** res, int nrows, int ncols)
{
	//Complex** res=Create2dArray(nrows,ncols);
	if(rank==0)
	{	
		for(int tempRank=1; tempRank<numtasks; tempRank++)
		{	
			rc=MPI_Send(&(res[0][0]), 2*nrows*ncols, MPI_COMPLEX, tempRank, 0, MPI_COMM_WORLD);
			if(rc!=MPI_SUCCESS)
			{
	...
	{	
		MPI_Status status;
		rc=MPI_Recv(&(res[0][0]), 2*nrows*ncols, MPI_COMPLEX, 0, 0, MPI_COMM_WORLD, &status);
		if(rc!=MPI_SUCCESS)
		{
...
{
	Complex** Htran=Create2dArray(nrows,ncols);
	for(int i=0; i<nrows; i++)
	{
		for(int j=0; j<ncols; j++)
		{
...
Complex** Transform1D(Complex** W, Complex** h, int nrows, int ncols, int flag2d)
{
  ...
  Complex** res1DFFT=Create2dArray(nrows,ncols); //2D Array to store final result of 1DFFT
	for(int i=0; i<nrows; i++)
	{
		for(int j=0; j<ncols; j++)
		{
  ...
  sendToAll(res1DFFT, nrows, ncols);
  return res1DFFT;
...
{ 
	Complex** res2DFFT=Create2dArray(nrows,ncols);	//2D Array to store final result of 2DFFT
	for(int i=0; i<nrows; i++)
	{
		for(int j=0; j<ncols; j++)
		{
	...
	sendToAll(res2DFFT, nrows, ncols);
	return res2DFFT;
}
